...
export default class UIManager {
 ...
 drawHUD() {
  ...
  drawTimer() {
    ...
    this.ctx.strokeStyle = timeWarning
      ? this.colors.error
  ...
  drawMenuButton() {
    ...
    const margin = 20;
    const spacing = 10;
    const x = this.canvasSize - size * 2 - margin - spacing;
    const y = this.canvasSize - size - margin;

    // Guardar info del botón
    this.menuButton = { x, y, width: size, height: size };
    ...
    // Icono de menú (3 líneas horizontales)
    const centerX = x + size / 2;
    const centerY = y + size / 2;
    const lineWidth = 20;
    ...
    this.ctx.beginPath();
    this.ctx.moveTo(centerX - lineWidth / 2, centerY - 8);
    this.ctx.lineTo(centerX + lineWidth / 2, centerY - 8);
    this.ctx.moveTo(centerX - lineWidth / 2, centerY);
    this.ctx.lineTo(centerX + lineWidth / 2, centerY);
    this.ctx.moveTo(centerX - lineWidth / 2, centerY + 8);
    this.ctx.lineTo(centerX + lineWidth / 2, centerY + 8);
    this.ctx.stroke();
  ...
  /**
   * Detecta click en botón de menú
   */
  checkMenuButton(mouseX, mouseY) {
